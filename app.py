...
app = Flask(__name__)
app.secret_key = 'dhukandhar-secret-key-9999'

PRODUCTS_FILE = 'products.json'
CART_FILE     = 'cart.json'
ORDERS_FILE   = 'orders.json'
WISHLIST_FILE = 'wishlist.json'
SAVE_LATER_FILE = 'save_later.json'
...
def load_save_later():
    try:
        with open(SAVE_LATER_FILE, encoding='utf-8') as f:
            return json.load(f)['save_later']
    except:
        return []

def save_save_later(sl):
    with open(SAVE_LATER_FILE, 'w', encoding='utf-8') as f:
        json.dump({"save_later": sl}, f, indent=4, ensure_ascii=False)
...
def index():
    ...
    if badge:
        products = [p for p in products if p.get('badge', '').upper() == badge.upper()]
...
@app.route('/save_for_later/<int:pid>')
def save_for_later(pid):
    cart = load_cart()
    sl   = load_save_later()
    # Find in cart and move
    for item in cart:
        if item['product_id'] == pid:
            # Check if already in SL (update qty if so)
            existing = next((i for i in sl if i['product_id'] == pid), None)
...
@app.route('/move_to_cart/<int:pid>')
...
@app.route('/remove_saved/<int:pid>')
def remove_saved(pid):
    sl = load_save_later()
    sl = [i for i in sl if i['product_id'] != pid]
    save_save_later(sl)
    flash("Item removed from saved list.", "success")
    return redirect(url_for('cart_page'))
...
def place_order():
    ...
    order_id = 'DHUKANDHAR-' + uuid.uuid4().hex[:8].upper()
    order = {
        'id':         order_id,
...
@app.route('/order/return/<order_id>/<int:product_id>')
def return_order_item(order_id, product_id):
    orders = load_orders()
    for o in orders:
        if o['id'] == order_id:
            for item in o.get('items', []):
                if item['product_id'] == product_id and item.get('status') != 'Returned':
                    item['status'] = 'Returned'
                    flash(f"Item '{item['name']}' returned successfully.", 'success')
            
            # Check if all items are returned or cancelled
            if all(i.get('status') == 'Returned' for i in o.get('items', [])):
                o['status'] = 'Returned'
            save_orders(orders)
            break
...
@app.route('/order/cancel/<order_id>/<int:product_id>')
def cancel_order_item(order_id, product_id):
    orders = load_orders()
    for o in orders:
        if o['id'] == order_id:
            for item in o.get('items', []):
                if item['product_id'] == product_id and item.get('status') not in ['Returned', 'Cancelled']:
                    item['status'] = 'Cancelled'
                    flash(f"Item '{item['name']}' cancelled successfully.", 'success')
            
            # Check if all items are cancelled correctly
            if all(i.get('status') == 'Cancelled' for i in o.get('items', [])):
                o['status'] = 'Cancelled'
            elif all(i.get('status') in ['Cancelled', 'Returned'] for i in o.get('items', [])):
                o['status'] = 'Returned'
            save_orders(orders)
            break
    return redirect(url_for('orders_page'))
...
def inject_globals():
    ...
    try:
        ...
        free_delivery_active = subtotal >= 999
    except Exception as e:
        pass
    return {'year': datetime.now().year, 'global_free_delivery': free_delivery_active}
